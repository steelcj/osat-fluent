...
### 10.7 Language layer placement

The language layer is placed at the root of the subtree that varies by language, and no higher.
...
The entire tree is language-variant, including executables, so the language root sits at the top. The language root is the unit of translation, archiving, and birthing, and its top-level position reflects that.

#### In an OSAT Fluent tool repository

```text
docs/en/README.md
```

Only the documentation currently varies by language. The manager, version file, and wrapper templates are language-neutral. The language layer therefore sits inside the documentation directory, with future languages added as siblings (`docs/fr/`). Placing the language layer at the repository root would create a half-language-root, an `en/` directory containing nothing but docs while the tool itself sits language-neutral beside it, implying a variance that does not exist. It would also seed a potentially unlimited number of language roots at the repository root, where `docs/en/` contains that growth inside `docs/`.

#### One rule, different variance scopes

These are not two conventions but one rule applied to different variance scopes. The governance repository itself uses `en/docs/` and `en/utils/` because its content is documents and language-variant utilities, making it SAT-shaped by the same rule.
